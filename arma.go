...
func checkWhitelist() string {
	
	whitelist_url := "https://raw.githubusercontent.com/7Cav/service-level-configs/develop/novak/arma3/public/whitelist.sqf"

	resp, err := http.Get(whitelist_url)
	if err != nil {
		log.Fatalln(err)
	}
...
func goRVExtension(output *C.char, outputsize C.size_t, input *C.char) {
	...
	if extensionCallbackFnc != nil {
		...
		check_result := checkWhitelist()
		result := C.CString(check_result)
		defer C.free(unsafe.Pointer(result))
		var size = C.strlen(result) + 1
		if size > outputsize {
			size = outputsize
